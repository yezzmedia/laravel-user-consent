...
<div id="preferences-app">
    ...
    <div class="divide-y divide-gray-200 dark:divide-gray-800">
            ...
            <div class="p-4 sm:p-6">
                ...
                <div class="space-y-2">
                    @foreach($requiredCats as $cat)
                        <div class="flex items-start gap-3 p-3 bg-indigo-50/60 dark:bg-indigo-900/10 border-l-2 border-indigo-400 dark:border-indigo-500">
                            <span class="mt-0.5 flex-shrink-0 w-5 h-5 border-2 border-indigo-400 dark:border-indigo-500 bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center" aria-hidden="true">
                                <svg class="w-3 h-3 text-indigo-600 dark:text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>
                            </span>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 flex-wrap">
                                    ...
                                    <span class="inline-flex items-center gap-1 text-xs font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-100 dark:bg-indigo-900/50 px-2 py-0.5">
                    ...
                    @endforeach
                    <p class="text-xs text-gray-400 dark:text-gray-500 italic pt-1">
        ...
        @if($optionalCats)
            <div class="p-4 sm:p-6">
                <div class="flex items-center gap-2 mb-4">
                    <span class="w-1 h-4 bg-gray-300 dark:bg-gray-600"></span>
                    <h2 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                        {{ __('user-consent::messages.optional_heading') }}
                    </h2>
                </div>
                <div class="space-y-1" id="pref-categories">
                    @foreach($optionalCats as $cat)
                        <label class="flex items-start gap-3 px-3 py-3 border-l-2 border-transparent hover:border-gray-300 dark:hover:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition cursor-pointer" data-cat-slug="{{ $cat['slug'] }}">
                            <input
                                type="checkbox"
                                onchange="prefToggle('{{ $cat['slug'] }}', this.checked)"
                                {{ $cat['granted'] ?? false ? 'checked' : '' }}
                                class="cb-pref-input mt-0.5 border-gray-300 dark:border-gray-600 text-indigo-600 focus:ring-indigo-500"
                            />
                            <div class="flex-1 min-w-0">
                                ...
                                @if($cat['granted'] !== null)
                                    <span class="cb-pref-badge inline-block mt-1.5 text-xs font-medium px-2 py-0.5
        ...
        <div class="p-4 sm:p-6 flex flex-wrap gap-2">
            <button
                ...
                class="px-4 py-2 bg-white dark:bg-gray-800 border border-emerald-300 dark:border-emerald-700 text-emerald-700 dark:text-emerald-300 text-sm font-medium hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
                ...
                class="px-4 py-2 bg-white dark:bg-gray-800 border border-red-300 dark:border-red-700 text-red-700 dark:text-red-300 text-sm font-medium hover:bg-red-50 dark:hover:bg-red-900/20 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
                onclick="prefSave()"
                id="pref-btn-save"
                class="px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
                ...
                id="pref-btn-reset"
                class="px-4 py-2 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 text-sm font-medium hover:bg-red-50 dark:hover:bg-red-900/20 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
...
(function () {
    ...
    function prefFlash(message, type) {
        ...
        flashEl.className = 'mx-4 sm:mx-6 mb-2 p-3 text-sm font-medium border-l-2 ' +
            (type === 'success'
                ? 'border-emerald-400 bg-emerald-50 text-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-300 dark:border-emerald-500'
                : 'border-red-400 bg-red-50 text-red-800 dark:bg-red-900/20 dark:text-red-300 dark:border-red-500');
    ...
    function prefUpdateBadges() {
        document.querySelectorAll('#pref-categories .cb-pref-input').forEach(function (input) {
            ...
            badge.className = 'cb-pref-badge inline-block mt-1.5 text-xs font-medium px-2 py-0.5 ' +
                (isGreen
